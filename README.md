...
### 제작 통계 (워크플로우 1회 실행 기준)

| 항목 | 값 |
|------|----|
| 동원된 에이전트 | **62개** |
| 서브에이전트 토큰 | **약 3,121,072 토큰 (≈ 3.12M)** |
| 도구 호출 횟수 | **359회** |
| 소요 시간 | **약 61.8분 (1시간 1분 49초 / 3,708,673 ms)** |
| 최종 원고 분량 | **약 37,000자 (공백 제외, 프롤로그+10장)** |
| 전체 감수 결과 | 치명 이슈 4건 적발 → 4개 챕터 자동 타깃 수정 |

> 모델: Claude Opus · 실행 환경: Claude Code Dynamic Workflow
> (토큰·시간은 워크플로우 런타임이 보고한 실측치이며, 메인 오케스트레이션 루프는 별도)
